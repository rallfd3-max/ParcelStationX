...
| 检查项 | 结果 | 证据 |
| --- | --- | --- |
| `mvn clean test` | 通过 | 33 tests，0 failure/error |
| `mvn -Pintegration-test verify` | 构建通过但未完成真实验收 | 2 个 MySQL IT 因环境变量缺失跳过 |
| `npm run type-check` | 通过 | Vue TSC 无错误 |
| `npm run test` | 通过 | 6 个文件、15 个测试 |
| `npm run build` | 通过 | Vite production build 成功 |

覆盖认证权限、HTTP 错误、仓储快照、乐观换位与 409 回滚、场景映射、相机定位、拖拽状态及 500/1000 件性能基线。MySQL IT 已包含 V2 表结构和“入库待上架 → 换位 → 占用数变化 → 出库”，但本次未实际连接 MySQL。
