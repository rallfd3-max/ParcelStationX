...
export class DragController {
  ...
  private pending?: {parcelId:number;pointerId:number;x:number;y:number}
  ...
  private down=(event:PointerEvent)=>{ const hit=this.cast(event,[...this.index.parcels.values()]); const id=this.index.parcelId(hit??null); if(!id)return; this.parcel=this.index.parcels.get(id); if(!this.parcel)return; this.origin.copy(this.parcel.position);this.pending={parcelId:id,pointerId:event.pointerId,x:event.clientX,y:event.clientY};this.select(id);this.element.setPointerCapture?.(event.pointerId) }
  private move=(event:PointerEvent)=>{ if(this.pending&&this.state.phase==='idle'){if(Math.hypot(event.clientX-this.pending.x,event.clientY-this.pending.y)<5)return;this.state=startDrag(this.pending.parcelId);this.controls.enabled=false}if(this.state.phase!=='dragging'||!this.parcel||!this.state.parcelId)return; const hit=this.cast(event,[...this.index.slots.values()]); const slotId=hit?.userData.slotId as number|undefined; const valid=Boolean(slotId&&this.available(slotId,this.state.parcelId)); this.state=targetDrag(this.state,slotId??null,valid); this.clearHighlight(); if(hit instanceof Mesh){this.highlighted=hit;const material=Array.isArray(hit.material)?hit.material[0]:hit.material;if(material&&'color'in material){this.originalColor=(material.color as Color).clone();(material.color as Color).set(valid?0x35d98b:0xff5269)}} if(valid&&hit)this.parcel.position.copy(hit.position) }
  private up=async()=>{this.pending=undefined;if(this.state.phase!=='dragging'){this.parcel=undefined;return}this.state=submitDrag(this.state); const {parcelId,targetSlotId}=this.state; if(this.state.phase==='pending'&&parcelId&&targetSlotId){const ok=await this.commit(parcelId,targetSlotId);if(!ok)await this.rollback();else this.finish()}else await this.rollback() }
  ...
  private finish(){this.clearHighlight();this.controls.enabled=true;this.pending=undefined;this.parcel=undefined;this.state=idleDragState()}
