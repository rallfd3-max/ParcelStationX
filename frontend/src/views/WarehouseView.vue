...
import { WarehouseDragScroller } from '@/features/warehouseDragScroll'
import { groupShelvesByZone } from '@/features/management'
const store = useWarehouseStore()
const route = useRoute()
const moveTarget = ref<number | null>(null)
const shelfCanvas = ref<HTMLElement | null>(null)
let dragScroller: WarehouseDragScroller | null = null
const inbound = reactive({ trackingNo: '', courierCompany: '顺丰', customerMobile: '13900000001', remark: 'Web 验收入库' })
onMounted(async () => { await store.refresh(); const requested=Number(route.query.parcelId); if(requested) store.selectParcel(requested); if(shelfCanvas.value) dragScroller=new WarehouseDragScroller(shelfCanvas.value); window.addEventListener('blur', finishDrag); window.addEventListener('keydown', cancelOnEscape) })
onBeforeUnmount(() => { finishDrag(); window.removeEventListener('blur', finishDrag); window.removeEventListener('keydown', cancelOnEscape) })
const shelves = computed(() => store.snapshot?.shelves ?? [])
const zones = computed(() => groupShelvesByZone(shelves.value))
...
function dragOver(event: DragEvent) { if (store.draggingParcelId && typeof event.clientY === 'number') dragScroller?.update(event.clientY) }
function stopAutoScroll() { dragScroller?.stop() }
function finishDrag() { dragScroller?.stop(); store.draggingParcelId = null }
function cancelOnEscape(event: KeyboardEvent) { if (event.key === 'Escape') finishDrag() }
async function drop(slot: ShelfSlot, event: DragEvent) { const id = Number(event.dataTransfer?.getData('text/plain') || store.draggingParcelId); finishDrag(); if (id && store.isSlotAvailable(slot, id)) await store.relocate(id, slot.id) }
...
<template>
  <section class="page warehouse-page">
    ...
    <div v-else-if="store.snapshot" class="warehouse-layout">
      <aside class="waiting-panel"><header><strong>待上架</strong><span>{{ visibleWaiting.length }}</span></header><div v-if="!visibleWaiting.length" class="empty">暂无待上架快件</div><button v-for="parcel in visibleWaiting" :key="parcel.id" class="parcel-card" draggable="true" @dragstart="dragStart(parcel.id, $event)" @dragend="finishDrag" @click="store.selectParcel(parcel.id)"><b>{{ parcel.trackingNo }}</b><small>{{ parcel.courierCompany }} · v{{ parcel.version }}</small></button></aside>
      <div ref="shelfCanvas" class="shelf-canvas" @dragover.prevent="dragOver" @dragleave.self="stopAutoScroll"><section v-for="zone in zones" :key="zone.zone" class="zone-group"><h2>{{zone.zone}} 区 <small>{{zone.shelves.length}} 架</small></h2><article v-for="shelf in zone.shelves" :key="shelf.id" class="shelf-block"><header><div><b>{{ shelf.shelfCode }}</b><small>{{ shelf.zone }}</small></div><span>{{ shelf.occupied }}/{{ shelf.capacity }}</span></header><div class="slot-grid"><button v-for="slot in slotsFor(shelf.id)" :key="slot.id" class="slot" :class="{ occupied: parcelAt(slot.id), disabled: !slot.enabled, available: store.draggingParcelId && store.isSlotAvailable(slot, store.draggingParcelId), invalid: store.draggingParcelId && !store.isSlotAvailable(slot, store.draggingParcelId) }" :disabled="!slot.enabled" :draggable="Boolean(parcelAt(slot.id))" @dragstart="parcelAt(slot.id) && dragStart(parcelAt(slot.id)!.id, $event)" @dragend="finishDrag" @dragover.prevent="dragOver" @drop.prevent="drop(slot, $event)" @click="parcelAt(slot.id) && store.selectParcel(parcelAt(slot.id)!.id)"><small>{{ slot.slotCode.split('-').slice(-2).join('-') }}</small><b v-if="parcelAt(slot.id)">{{ parcelAt(slot.id)?.trackingNo }}</b><span v-else>{{ store.draggingParcelId ? (store.isSlotAvailable(slot, store.draggingParcelId) ? '可放置' : '不可放') : '空闲' }}</span></button></div></article></section></div>
